...
class TestCorpusVisualise:
    def test_visualise_single_crate(self):
        import tempfile

        corpus = Corpus(MINIMAL)
        with tempfile.TemporaryDirectory() as tmpdir:
            filepath = str(Path(tmpdir) / "grid.html")
            result = corpus.visualise(filepath=filepath)
            assert result == filepath
            content = Path(filepath).read_text()
            assert 'id="grid"' in content
            assert '"grid": true' in content or '"grid":true' in content

    def test_visualise_multiple_crates(self):
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert result == filepath
            content = Path(filepath).read_text()
            # Both crates should appear in the grid data
            assert content.count('"graphData"') == 2

    def test_visualise_auto_columns(self):
        import tempfile

        corpus = Corpus(MINIMAL, SECOND, QUIRKY)
        with tempfile.TemporaryDirectory() as tmpdir:
            filepath = str(Path(tmpdir) / "grid.html")
            corpus.visualise(filepath=filepath)
            content = Path(filepath).read_text()
            # 3 crates → ceil(sqrt(3)) = 2 columns
            assert "repeat(2," in content

    def test_visualise_explicit_columns(self):
        import tempfile

        corpus = Corpus(MINIMAL, SECOND, QUIRKY)
        with tempfile.TemporaryDirectory() as tmpdir:
            filepath = str(Path(tmpdir) / "grid.html")
            corpus.visualise(filepath=filepath, columns=3)
            content = Path(filepath).read_text()
            assert "repeat(3," in content

    def test_visualise_contains_crate_labels(self):
        import tempfile
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            corpus.visualise(filepath=filepath)
            content = Path(filepath).read_text()
            assert "minimal-crate" in content
            assert "second-crate" in content

    def test_visualise_returns_html_when_no_filepath(self):
        corpus = Corpus(MINIMAL)
        result = corpus.visualise()
        html_str = result.data if hasattr(result, "data") else str(result)
        assert 'id="grid"' in html_str
